...
knitr::opts_chunk$set(
  ...
  stop = FALSE, fig.height = 4, fig.width = 6, 
...
Discrete hazard models are popular corporate distress models.
<small>@Shumway01, @Chava04, and @Campbell08 had `r 2395L + 846L + 1635L`
...
$$
E\LPV{\phi\LP{\vect X_{t-1:t}}}{\vect\theta^{(k)}}, \qquad 
  E\LPV{\psi\LP{\vect X_t}}{\vect\theta^{(k)}}
$$
...
$$
\PF{\vect x}{t}{i} \sim 
q_t\LPV{\cdot}{\PF{\vect x}{t-1}{i},\vect y_t}
$$
<!--html_preserve-->
...
$$
\begin{align*}
Q_1\LPV{\vect\beta}{\vect\theta^{(k)}} &= E\LPV{\sum_{t = 1}^d\sum_{i \in R_t}
    \LP{h\LP{\eta_{it}\LP{\vect X_t}}T\LP{y_{it}} - A\LP{\eta_{it}(\vect X_t))}
        + B(y_{it})}}{\vect \theta^{(k)}} \\
\eta_{it}\LP{\vect x_t} &= 
    \vect\beta^\top\vect z_{it} + o_{it} + \vect x_t^\top\vect u_{it}
...
<small>**Bootstrap filter** from @Gordon93.</small>
...
<small>As suggested in @Pitt99 and used in @Fearnhead10.</small></div></div>

</diV>

## Proposal distribution
<div class="left">
Previous is $\bigO{Nn_tp^2}$ where $p$ is the dimension of $\vect x_t$. Instead 
make the expansion once using
...
plot(head(f1$effective_sample_size$smoothed_clouds, -1), type = "h", 
     ylab = "Effective sample size", xlab = "t")
```
...
plot(head(f2$effective_sample_size$smoothed_clouds - 
     ...
     ylab = "Diff effective sample size", xlab = "t")
```
...
<small><p>Slides are on <a href="https://github.com/boennecd/Talks">github.com/boennecd/Talks</a>.</p>
