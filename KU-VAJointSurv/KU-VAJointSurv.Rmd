...
The population registers, laboratories and research questions 
...
<p class = "fragment"> 
Allows for non-proportional fixed and random effects.</p>
...
## The psqn Package
<div class = "w-small">
A BFGS approximation is made of the Hessian of each $\tilde l_i$. 
<p class = "smallish"> 
...
<p class = "smallish">
with a block diagonal preconditioner.</p>
</div>
...
The user just needs a thread-safe implementation of $\tilde l_i$ and its 
gradient.</p></div>
...
<p class = "fragment">
Simulation study to check the bias of the VA, the observed information matrix 
and the computation.</p>
...
colnames(comp) <- c(
  ...
  "$\\gamma_{12}$",
  "$\\alpha_1$", "$\\lVert\\widehat{\\mat\\Psi} - \\mat\\Psi\\rVert$")
...
evaluate or be harder to optimize. A reasonably tractable example is 
the skew normal distribution @ormerod11.</p>
</div>
