...
$$
l_i(\vec\theta) = 
  \int h_i(\vec Y_i;\vec u, \vec\theta)g_i(\vec u; \vec\theta)\der\vec u
$$
...
$\vec\omega_i \in \Omega_i$ and use
...
Typically much easier computationally to evaluate.
...
$$
\argmax_{\vec\theta} l(\vec\theta) = \sum_{i = 1}^m l_i(\vec\theta)
$$

with 

$$
\argmax_{\vec\theta,\vec\omega_1, \dots, \vec\omega_m} 
...
$\lvert\vec\theta\rvert + \sum_{i = 1}^m \lvert\vec\omega_i\rvert$ instead 
of $\lvert\vec\theta\rvert$.

The Hessian is sparse though: partially separable.
...
<p class = "fragment">
Quasi-Newton methods like BFGS creates an approximation of the Hessian of 
$\tilde l$.</p>

<p class="fragment">
BFGS does not preserve the sparsity of $\tilde l$.</p>
...
Solve the normal equation using conjugate gradient. 
<p class = "smallish">
Fast as we preserve the sparsity.
</p></div>

## Comparing with BFGS
$\vec\theta$ has $p$ elements, 
$\vec\omega_i$ have $s$ elements, and 
at most $n_{cg}$ conjugate gradient iterations.

<div class = "fragment">
The ratio of flops between BFGS and the new methods is (roughly)
...
for large $m$ and $s$ slightly larger than $p$. Favorable if $n_{cg} \ll m$.</div>
...
Supports computation in parallel using OpenMP for C++ implementations.
...
sim_dat <- function(n_cluster = 100L, cor_mat, beta, n_obs = 10L, 
  ...
  out <- replicate(n_cluster, {
    ...
    if(model == "binomial_logit"){
      prob <- 1/(1 + exp(-eta))
      nis <- sample.int(3L, n_obs, replace = TRUE)
...
$$\begin{align*}
Y_{ij} &\sim \text{Bin}\left(h_{ij}, (1 + \exp(-\eta_{ij}))^{-1}\right) 
  & h_{ij} &\sim \text{Unif}(\{1, 2, 3\}) \\
...
stopifnot(all(apply(keep == 0, 2L, all)))
...
BFGS had convergence problems.

## Bias: Standard Deviations

```{r drop_extreme_bfgs_res}
# drop extreme BFGS results
do_drop <- apply(abs(stddev_err["GVA BFGS", , ]) > 10, 2L, any)
message(sprintf("Fraction removed of BFGS %.2f", mean(do_drop)))
fix_err   ["GVA BFGS", , do_drop] <- NA_real_
stddev_err["GVA BFGS", , do_drop] <- NA_real_
cor_err   ["GVA BFGS", , do_drop] <- NA_real_
...
Excluded solutions for BFGS where it did not converge.
...
tis_stats <- lapply(tis_stats, function(x) {
  ...
  colnames(x) <- "Computation time (secs)"
  x
})
