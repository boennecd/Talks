...
\Phi^{(K)}(\vec x;\vec\mu,\mat\Sigma) &= 
  ...
  \phi^{(K)}(\vec z;\vec\mu,\mat\Sigma)\der z_1 \cdots \der z_K \\
\end{align*}
$$
The standard case
$$
...
Mixed multinomial
<p class="smallish">
Outcomes $Y_i \in \{1,\dots,c\}$ are conditionally 
...
$$
l(\vec\beta,\mat\Sigma) = \log\int p(\vec u, \vec y) 
...
$\bigO{n^3 + n^2K + nK^2}$,
<p class = "smallish">
and typically not $\bigO{n^3}$ in practical examples it seems.</p>
</div>
...
<p class = "fragment">
Recursive application of quadrature rule using $b$ values for each of 
the $K$ dimensions.</p>
...
p(\vec v_1\mid \vec V_2 \leq \vec v_2) &=   
  ...
  {\Prob(\vec V_2 \leq\vec v_2)}\\
&\hspace{-60pt}= \phi^{(k_1)}(\vec v_1; \vec \xi_1, \mat\Xi_{11}) \\
&\hspace{-20pt}\cdot
  \frac
  {\Phi^{(k_2)}\left(
  \vec v_2
