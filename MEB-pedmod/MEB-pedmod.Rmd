...
Work on variational approximations in biostatistic.
<p class = "smallish">
A way of approximating integrals when estimating models. Often very fast.</p>
</div>

<div class = "w-small fragment">
Has almost nothing to with today's topic!
<p class = "smallish">
...
<div class ="fragment w-small">
Other outcomes have been studied in the department. TODO: add more studies.
<p class = "smallish">
E.g. pre-eclamptic events, 
melanoma onset, preterm birth, and autism spectrum disorder.
[@Pawitan04; @Lindstroem06; @Svensson09; @yip2018; @Bai19].</p>
</div>
...
where $\epsilon_{ij}$ is standard normally distributed. 
...
$$
<p class = "smallish">A GLM. $\Phi$ is the standard normal CDF.</p>
</div>
...
<section class="center-horiz">
<h2>Liability-Threshold Models</h2>
<!--/html_preserve-->
...
plot(ex_dat)
image_plot <- function(K, ...){
  col <- gray.colors(n = 20, start = 1, end = 0)
...
<p class = "smallish">
with $\mat C_{ig}$ being two times the kinship matrix and $\mat C_{ie}$ is 
a matrix as defined before.</p>
</div>
...
May want paternal effect or maternal effect. 
...
(\epsilon_{i1}, \dots, \epsilon_{in_i})^\top & 
  ...
  \mat\Sigma(\vec\sigma)) \\
\mat\Sigma(\vec\sigma) &= \sum_{k = 1}^K\sigma_k^2 \mat L_{ik} 
\end{align*}
$$
...
<p class = "smallish">
Used by @Pawitan04, @Lindstroem06, @Svensson09, @yip2018, and @Bai19. </p>
</div>
...
To simplify the computation, they do not use all data by
 ...
 - ignoring information from the pedigree, and
...
<p class = "fragment">
Method with gradient approximations.</p>

<p class = "fragment">
A faster approximation and implementation that supports computation in 
parallel.</p>
...
<p class = "fragment">
At a high level, mention what have done before and how we extended it.</p>
...
<p class = "smallish"> 
and their ancestor and descendants.</p>
</div>
...
by removing 3.164 parent-child links
<p class = "smallish">
out of 3.944.737 (`r 100 * 3164 / 3944737` percent reduction)!</p>
</div>
...
<p class = "fragment">
Bonus: The CDF dimension is equal to the number of censored individuals.</p>
...
<div class = "w-small">
Recall: I work on variational approximations in biostatistic.
<p class = "smallish">
A way of approximating integrals when estimating models. Often very fast.</p>
</div>
...
<p class = "smallish">
In the thousands or even a millions.</p>
</div>
...
accounting for diabetic complications, glycated hemoglobin (HbA1c) and 
medications.</p>
