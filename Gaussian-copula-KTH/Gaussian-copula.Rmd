...
I.e. a binary variable with borders $0 < \Phi(\mu_{j1}) < 1$.
</div>
...
\vec V_i \sim N^{(h)}\left(
  ...
  \begin{pmatrix} \sigma^2_{gg} & \vec\sigma_{g(-g)} \\ 
...
$$
\begin{align*}
\int \phi(z;\mu_g;\sigma_{gg}^2) \Phi^{(h - 1)}\bigg(&\vec a + \vec dz - \mat K(z - \mu_g), \vec b + \vec dz - \mat K(z - \mu_g); \\
& \vec\mu_{(-g)}, \mat\Sigma_{(-g)(-g)} - \frac{\vec\sigma_{(-g)g}\vec\sigma_{(-g)g}^\top}{\sigma_{gg}^2}\bigg)\der z \\
\end{align*}
$$

with $\mat K = \vec\sigma_{(-g)g}/\sigma_{gg}^2$ and 
...
$$
\Phi^{(h - 1)}\bigg(\vec a, \vec b; 
        \vec\mu_{(-g)} - \vec d\mu_g, \mat\Sigma_{(-g)(-g)} + \vec d\vec d^\top\sigma_{gg}^2\bigg)
$$
