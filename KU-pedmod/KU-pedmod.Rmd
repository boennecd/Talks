...
The probit link gives a computationally attractive model 
[@Christoffersen21Probit]!</p></div>
...
<p class = "smallish">
Darker colors are further from zero. The upper triangle is omitted.</p>
</div>
...
<p class = "smallish"> 
Our version better exploits auto vectorization from compilers.</p></div>
...
<p class = "smallish">
for each of 250 ten dimensional integrals.</p></div>
...
computation time in seconds for MCMCglmm.</p>
...
transform_dat <- function(data, new_order){
  out <- data[new_order, ]
}
new_order <- order(new_dat$id %in% partition$set_1)
...
K_old <- kinship(
  with(transform_dat(dat, new_order), 
       pedigree(id = id, dadid = dad, momid = mom, sex = sex)))
K_new <- kinship(
  with(transform_dat(new_dat, new_order), 
       get_pedigree_mock(id = id, dadid = dad, momid = mom, sex = sex)))
...
## Survival Data 
<div class = "w-small">
The IPCW based estimator used by @Holst16 
<p class = "smallish">
...
<p class = "smallish">
Fast approximations exists and derivatives are easily computed 
[@Genz04; @Vaida09].
</p></div>
...
\begin{align*}
Y_{ij} &= \begin{cases}
  1 & \vec x_{ij}^\top\vec\beta + \epsilon_{ij0} + \sum_{k = 1}^K\sigma_k(\vec z_{ij})\epsilon_{ijk} > 0 \\
  0 & \text{otherwise}
\end{cases} \\
\epsilon_{ij0} &\sim N(0,1) \\
(\epsilon_{i1k}, \dots, \epsilon_{in_ik})^\top & 
