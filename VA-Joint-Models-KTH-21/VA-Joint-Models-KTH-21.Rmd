...
<p class = "fragment">
Is the risk related to long periods of higher than normal density, the 
current density only, or large changes over a short period of time?</p>

<p class = "fragment">
We need a model for the density, *the marker*, and the risk of 
breast cancer, *the survival time*.</p>

## Markers

<div class = "w-small">
We have a number of individuals where we observe markers at different points 
in time. 
<p class = "smallish">E.g. breast density.</p></div>
...
<p class = "smallish">
Observations for two individuals.</p>
...
<p class = "smallish">
Observations for two individuals with their mean curves.</p>

</section>
<section class="center-horiz" data-transition="fade-in fade-out">
<h2>Example: Population Curves</h2>
...
<p class = "smallish">E.g. diagnosis of disease, 
onset of a disease, and death from a disease.</p>
</div>

<p class = "fragment">
The deviation from the markers from the population may be linked to 
higher or lower risk of the event for the individuals than the population. </p>
...
lines(xs, particular_curve, col = "Gray28", lty = 3)
grid()
...
Given the hazard, $h_i$, we can compute the density of 
$T_i = t$ and probabilities for events like $T_i > t$ and $T_i\in (t, v)$. 
...
<p class = "fragment">
$\omega^\top \vec b(t)$: the population log hazard function.</p>
...
The likelihood is intractable. It requires evaluation of expression like 

$$
E\left(
  g(\vec U_i)h_{ij}(t\mid \vec U_i, \vec\xi_i)
  \exp\left(-\int_0^t h_{ij}(s\mid \vec U_i, \vec\xi_i)ds\right)\right)
$$
...
Select some distribution $q_{\vec\theta}$ for $\theta\in \Theta$. If $l_i$ is 
the log likelihood term of individual $i$ with outcome $\vec y_i$

$$
\begin{align*}
l_i(\vec\zeta) &= \log p_i(\vec y_i) 
  = \log \int q_{\vec\theta}(\vec U)
    \log \left(\frac{p_i(\vec y_i, \vec U)\big/ q_{\vec\theta}(\vec U)}
    {p_i(\vec U\mid\vec y_i)\big/ q_{\vec\theta}(\vec U)}\right) d \vec U\\
  &= \log \int q_{\vec\theta}(\vec U)
    \left(
      \log \left(\frac{p_i(\vec y_i, \vec U)}
      {q_{\vec\theta}(\vec U)}\right) d \vec U +
      \log \left(\frac{q_{\vec\theta}(\vec U)}
      {p_i(\vec U\mid\vec y_i)}\right)
    \right)d \vec U \\
  &\geq \log \int q_{\vec\theta}(\vec U)
    \log \left(\frac{p_i(\vec y_i, \vec U)}
    {q_{\vec\theta}(\vec U)}\right) d \vec U 
...
<p class = "fragment">
Equality if $p_i(\vec U\mid\vec y_i) = q_{\vec\theta}(\vec U)$.</p>

## Approximate Maximum Likelihood

Let $\vec\zeta$ be the model parameters. Then we assume that 
...
For a given variational distribution $q_{\vec\theta}$, we need to be 
able to easily evaluate 

 - the entropy: $-E_{q_{\vec\theta}}(\log q_{\vec\theta}(\vec U))$.
 - the first and second moment: $E_{q_{\vec\theta}}(\vec U)$ and 
   $E_{q_{\vec\theta}}(\vec U\vec U^\top)$. 
 - the moment generating function: $E_{q_{\vec\theta}}(\exp(\vec t^\top\vec U))$.

<p class = "fragment">
Not strong requirements!</p>
...
<div class = "w-small fragment">
Can be extended with e.g. with multivariate t-distribution, 
skew normal distribution, or combination thereof. 
<p class = "smallish">
Will give a tighter lower bound, but may be harder to evaluate or harder to 
optimize (more saddle points or local maximums).
</p></div>
...
<p class = "fragment">
Supports left-truncation, partly observed markers, and much more.</p>
...
<p class = "fragment">
There is one (terminal) time to event outcome for each individual.</p>
...
<div class = "w-small">
The joineRML makes no assumption about the baseline hazard, 
...
<p class = "fragment">
One (terminal) time to event outcome for each individual and 
a model for the observation times.</p>
...
sim_res <- lapply(seeds, function(s){
  ...
  message(sprintf(
    "\nLower bound (starting value), conv code, time: %12.2f %12.2f %2d %8.2f",
...
fmt_bias(bias_fixef_surv, mult)
```

<p class = "smallish">
Bias estimates for the time-varying fixed effects of the survival outcomes, 
$\vec\delta_1$ and $\vec\delta_2$, are shown above multiplied by `r mult`.
The SE columns are the standard errors.
</p>
...
fmt_bias(bias_fixef_vary_surv, mult)
```

<p class = "smallish">
Bias estimates for the fixed effects of the survival outcomes, 
$\vec\omega_1$ and $\vec\omega_2$, are shown above multiplied by `r mult`.
The SE columns are the standard errors.
</p>
...
fmt_bias_vcov <- \(x, mult){
  out <- cbind(x$bias, NA, x$se)
  colnames(out) <- character(NCOL(out))
  colnames(out)[c(1L, 2L + NCOL(x$bias))] <- c("Bias", "SE")
  rownames(out) <- character(NROW(out))
  knitr::kable(mult * out)
}

fmt_bias_vcov(comp_bias_vcov(est_vcov_vary, vcov_vary), mult)
```

<p class = "smallish">
Bias estimates for the covariance matrix of the random effects, $\Psi$,
are shown above multiplied by `r mult`. The last columns show the standard 
errors.
</p>
...
fmt_bias_vcov(comp_bias_vcov(est_vcov_marker, vcov_marker), mult)
```
...
fmt_bias_vcov(comp_bias_vcov(est_vcov_surv, vcov_surv), mult)
```
...
<div class = "fragment w-small">
The psqn packages needs to be extended to allow for constraints 
...
## Variational Approximations
<div class = "w-small">
Considering other variational approximations will be interesting if bias is 
an issue in some models.
<p class = "smallish">
Recall that the requirements on the variational distribution, $q_{\vec\theta}$,
are not large!
</p></div>

<div class = "w-small fragment">
Can consider clustered individuals. 
Estimation can be fast with restricted Gaussian variational approximations.
<p class = "smallish">
...
<a href="https://rpubs.com/boennecd/VA-Joint-Models-KTH-21">rpubs.com/boennecd/VA-Joint-Models-KTH-21</a>.</p>
