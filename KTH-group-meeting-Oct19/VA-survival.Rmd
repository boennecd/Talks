---
title: "Variational Approximations in Surival Analysis"
...
<p class="fragment">
Time zero is time of diagnosis, onset, or birth date.</p>

<p class="fragment">
Some event are (right) censored.</p>
...
<p class="smallish">
e.g., short trail period or low event rate.</p>
</div>
...
$$
\lambda\Cond{t}{x} = \lambda_0(t)\exp\left(\vec\beta^\top\vec x\right)
$$
<div class="w-small">
...
g\left(S\Cond{t_{ki}}{\vec x_{ki}, \vec z_{ki}, \vec u_k}\right) &= 
  ...
  &= g\left(S_0(t_{ki}; \vec x_{ki})\right) + \vec z^\top\vec u_k \\
\vec U_k &\sim h(\vec \theta)
\end{align*}
$$
<div class="w-small">
$\vec z$ is known covariates and $\vec u_k$ is group $k$'s random effect.
<p class="smallish">
...
<p class="smallish">
Fast, scales well, but may perform poorly e.g., small groups ($n_k$ small).
</p></div>
...
## Adaptive Gaussian Quadrature
Apply Gauss-Hermite quadrature for each coordinate of $\vec s$.
...
A lower bound of the log-likelihood is
...
l_k(\vec\beta, \mat\Sigma) &= \log \int
  ...
  \der \vec u \\
h_k(\vec\beta, \mat\Sigma, \vec u) &=
  \vec d^\top \log \lambda\Cond{\vec t_k}{\mat X_k, \mat Z_k, \vec u} \\
  &\hspace{20pt} + \vec 1^\top\log S\Cond{\vec t_k}{\mat X_k, \mat Z_k, \vec u}
\end{align*}
$$
...
$$\left(\vec d^\top \log \lambda\Cond{\vec t_k}{\mat X_k, \mat Z_k, \vec u} + \vec 1^\top\log S\Cond{\vec t_k}{\mat X_k, \mat Z_k, \vec u}\right)\phi(\cdot)$$
</div>
...
\lambda\Cond{s}{\vec x_{ki}, \vec z_{ki}, \vec u_k}
  ...
  \exp\left(f(s, \vec z_{ki})^\top\vec u_k\right)
$$
<p>where $f$ is **not** applied elementwise.<span class = "fragment" data-fragment-index=1> Then</span></p>
...
&\hspace{-40pt}= -\int_0^{t_{ki}} \lambda_0(s ;\vec x_{ki})
  \exp\left(f(s, \vec z_{ki})^\top\vec u_k\right) \der s \\
\log\lambda\Cond{t_{ki}}{\vec x_{ki}, \vec z_{ki}, \vec u_k}
  &= \log \lambda_0(t_{ki};\vec x_{ki}) + 
  f(t_{ki}, \vec z_{ki})^\top\vec u_k
\end{align*}$$
</div>
...
&=- \int_0^{t_{ki}} \lambda_0(s ;\vec x_{ki})
  \bigg(\int
  \exp\left(f(s, \vec z_{ki})^\top\vec u\right) \\
&\hspace{110pt}\cdot
...
<a href="https://github.com/boennecd/Talks/KTH-group-meeting-Oct19/notes.pdf">github.com/boennecd/Talks/KTH-group-meeting-Oct19/notes.pdf</a>.</p>
