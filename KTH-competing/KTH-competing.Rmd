...
Moreover, for interesting examples, 
$\lim_{t\rightarrow\infty}F_{jk}(t) < \lim_{t\rightarrow\infty} F_j(t) = 1$.
</div>
...
```{r illustration, fig.height = knitr::opts_chunk$get("fig.width") * 2 / 5}
...
local({
  ...
  par(mar = c(4, 5, 1, 1), mfcol = c(1, 2), cex = .8, mgp = c(2, 1, 0))
  xs <- seq(0, 2, length.out = 1000)
  
  for(i in 1:2){
    vals <- sapply(xs, \(x) probs[i] * pnorm(
      -coef_traject[1, i] * atanh((x - delta / 2) / (delta / 2)) - 
        coef_traject[2, i]))
    plot(
      xs, vals, xlim = c(1e-8, delta), ylim = c(0, 1), bty = "l",  
      xlab = "Time", type = "l",
      ylab = bquote(F[paste("j", .(i))](t) == P(T[j]^"*" <= t, epsilon[j]^"*" == .(i))),
      yaxs = "i", xaxs = "i", xaxt='n')
    grid()
    
    abline(h = max(vals), lty = 2)
    text(1.5, max(vals) + .06, 
         labels = bquote(P(T[j]^"*" <= tau, epsilon[j]^"*" == .(i))))
    
    axis(1, c(0, 2), labels = c(0, expression(tau)))
  }
})
```
...
$$
<p class = "smallish">
See @Varin11 for an overview of composite likelihood.</p></div>
...
<p class = "fragment">
The composite likelihood term is a bit tedious to implement with 
$3^2 - 2 = 7$ types of composite likelihood terms.</p>
