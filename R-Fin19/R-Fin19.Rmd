...
$$
\begin{aligned}
\log\lambda_{it} &= \vect\beta^\top \vect x_{it} + \vect\gamma^\top \vect z_t + A_k \\
A_k &\sim \theta A_{k-1} + \epsilon_k \\ 
...
\log\lambda_{it} &= \vect\beta^{(1)\top}\vect x_{it}^{(1)} +  
    ...
    \vect A_k^\top\vect u_{it} \\
\vect A_k &\sim F\vect A_{k-1} + \vect \epsilon_k \\ 
\vect \epsilon_k&\sim \vect N(\vect 0, Q) \\
\vect x_{it} &= 
     \left(\vect x_{it}^{(1)\top}, \vect x_{it}^{(2)\top}\right)^\top
\end{aligned}    
$$

$\vect A_k \in \mathbb{R}^p$ is low dimensional and some elements in $\vect u_{it}$ and 
...
\log\lambda_{it} &= \color{gray}{\vect\beta^{(1)\top}\vect x_{it}^{(1)} +  
    ...
    \vect\beta^{(2)\top} \vect f(\vect x_{it}^{(2)}) + 
    \vect A_k^\top\vect u_{it} \\
\color{gray}{\vect A_k} &\sim F \color{gray}{\vect A_{k-1} + \vect \epsilon_k} \\ 
\color{gray}{\vect \epsilon_k}
...
<p class="gray">$\vect A_k \in \mathbb{R}^p$ is low dimensional and some elements in $\vect u_{it}$ and 
...
TODO: insert small example where a model is estimated.  
...
const_fit <- survreg(
  Surv(tstop - tstart, death == 2L) ~ age + edema + log(albumin) + 
    log(bili) + log(protime), data = pbc2, dist = "exponential")

pf_fit <- PF_EM(
  fixed = Surv(tstart, tstop, death == 2L) ~ age + edema + log(albumin) + 
    log(protime) - 1, random = ~ log(bili), 
  data = pbc2, by = 50L, max_T = max(pbc2$tstop), model = "exponential", 
  id = pbc2$id, order = 1L, Q_0 = diag(4, 2), type = "RW", Q = diag(1e-4, 2),
  control = PF_control(
    N_fw_n_bw = 200L, N_smooth = 1000L, N_first = 1000L, eps = 1e-4,
    nu = 6, smoother = "Fearnhead_O_N", n_threads = 6, n_max = 1L), 
  trace = 1L)
```
